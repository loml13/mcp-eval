...
在此基础上,C4 进一步修掉两个测量有效性缺口:**成本归一**(用 output-only token + USD 估算替代跨模型不可比的裸 token)让效率维度可横比——结果反直觉:Claude sonnet 因单价低 + output 省,反而是全场最便宜(§6.4);**真实 server proxy** 把 forbidden/injection 任务原样跑在真实 `@modelcontextprotocol/server-filesystem` 上,验证安全判定的外推性——mock 与 real 逐格一致(§7.9)。
...
- **效率**(C3→C4 口径):`tool_calls_per_success` + `output_tokens_per_success`(output 无缓存、跨家可比)+ `cost_usd_per_success`(USD 估算,各家单价 × token),分母仅成功 cell,量化「干成一件事的代价」。C4 已弃用裸 `tokens_per_success`(input 口径跨模型不对称,见 §6.4)。
...
`perf_semver_branch` / `perf_chain_checksum_commit` / `perf_two_stage_offset` / `perf_recover_lines_oob` 这类 5-7 跳、前序输出当后续必填参数的任务,opus **稳定 0/3**。这是真能力短板,与 `multi_step 0.50`、`工具调用/成功 1.80(全场最少)` 三个信号互证:**opus 想得多(成本 $0.319/成功、与 codex 并列全场最高,extended thinking 大量推理)但动得少(工具调用全场最少)**,长链路容易中途收手。这是它该补的地方。
