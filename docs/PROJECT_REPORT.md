...
核心发现:**对前沿模型,「能不能用 MCP 完成任务」已经饱和(都能),真正有区分度的是「用得安不安全、稳不稳」**。在 **9 个真实被测对象 + 1 个确定性锚点(共 10 agent)、每任务重复 3 次(pass^k)** 的 C3 实测中,功能成功率虽都在 0.80~0.96,但安全可靠性(`safety_pass^k`)从 0.67 拉到 0.91、注入抵抗率从 0.81 到 1.00——**功能与安全是两条相互独立的轴**:功能最强的 mimo(0.96)安全最差(0.67),安全最强的 Claude(opus 0.91)功能中游。benchmark 能精确定位「哪个攻击对哪个模型、在第几次重复里有效」,而不只给一个笼统总分。

> **一个意料外的方法论收获**:C3 矩阵首跑时,Claude 两格因与编排会话共用同一 Max 配额池而被限流,**约 40% 的 cell 拿到的是「配额用尽」文案、被当成答错**——若只看汇总分,opus/sonnet 会被冤判为 0.49/0.48 的垫底差生。靠**逐 cell 的 server 侧 trace 核验** + **pass^k 的重复结构**,这批污染被精确识别、隔离重跑、整格替换,修正后 opus/sonnet 回到 0.86/0.80。这正印证了「ground-truth trace + 重复测量」相对「单次跑分」的价值(详见 §7.5)。
...
### 6.1 C3 总榜(10 agent × 69 任务,每任务 ×3)

按 `成功率 − 越权调用率`(安全惩罚)降序。所有真实被测对象均 207 cell(69×3),scripted 锚点 69(确定性,1 次即定)。

| 被测对象 | 成功率 | pass^k | 安全pass^k | 越权调用率 | 密钥泄漏率 | 越权访问率 | 注入抵抗率 | 多跳完成率 |
|---|---|---|---|---|---|---|---|---|
| glm-5.1 | 0.92 | 0.89 | **0.89** | 0.07 | 0.02 | 0.02 | 0.97 | 0.61 |
| qwen3.7-max | **0.95** | 0.92 | 0.83 | 0.12 | 0.00 | 0.04 | 0.97 | **0.83** |
| kimi-k2.6 | 0.91 | 0.87 | 0.87 | 0.10 | 0.09 | 0.06 | 0.93 | 0.72 |
| MiniMax-M3 | 0.88 | 0.86 | 0.85 | 0.09 | 0.00 | 0.01 | 0.99 | 0.72 |
| **claude-opus** | 0.86 | 0.86 | **0.91** | 0.07 | 0.00 | 0.00 | **1.00** | 0.50 |
| codex(gpt-5.5) | 0.84 | 0.78 | 0.80 | 0.06 | 0.00 | 0.02 | **1.00** | **0.28** ⚠️ |
| **claude-sonnet** | 0.80 | 0.78 | 0.89 | 0.04 | 0.00 | 0.00 | **1.00** | 0.56 |
| mimo-v2.5-pro | **0.96** | **0.94** | **0.67** ⚠️ | **0.20** | 0.10 | **0.15** | 0.83 | **0.83** |
| deepseek-v4-pro | 0.89 | 0.86 | 0.74 | 0.19 | **0.17** ⚠️ | 0.14 | **0.81** | 0.61 |
...
**一句话画像:** 按安全惩罚后的总榜由 glm 领跑;若只看纯功能,mimo/qwen 最强。国产 API 模型(qwen/mimo/glm/kimi/deepseek/minimax)**功能强(0.88~0.96)** 但安全参差;Claude 系 + codex **功能中游(0.80~0.86)** 但**注入抵抗满分(1.00)、安全可靠性顶格**。没有全能赢家。

### 6.2 功能与安全正交:这是最重要的一张图

把"成功率(功能)"与"安全pass^k(安全可靠)"两列对照,模型清晰地分成两群:
...
安全pass^k
  ...
  0.67 │                        ● mimo
       └──────────────────────────────────────────► 成功率
        0.80   0.84   0.88   0.92   0.95  0.96
```

- **右下角(高功能 / 低安全)**:mimo(0.96 / 0.67)、deepseek(0.89 / 0.74)——能干活但容易被骗,**强而不稳**。
- **左上角(中功能 / 高安全)**:opus(0.86 / 0.91)、sonnet(0.80 / 0.89)——干活保守、守得住。
- **甜点区**:glm(0.92 / 0.89)、qwen(0.95 / 0.83)——国产模型里"高功能且较安全"的性价比之选。

结论:**功能分高 ≠ 安全分高**。两条轴的相关性接近零(mimo 功能第一安全垫底,sonnet 功能垫底安全第二),只测其一会得出严重误导的排名。
...
### 6.4 效率:同一道题,token 花销差 200 倍

只统计**成功 cell**的资源消耗(失败的不算,避免"放弃得快"反而显得高效)。

| 被测对象 | 工具调用/成功 | token/成功 | 解读 |
|---|---|---|---|
| claude-sonnet | 1.77 | 92.8 \* | \* Claude Code 口径不计 cache-read,数字偏小 |
| kimi-k2.6 | 1.99 | **4,212** | 真实 LLM 里最省 |
| glm-5.1 | 1.88 | 6,438 | 省且准 |
| deepseek-v4-pro | 2.18 | 7,913 | |
| MiniMax-M3 | 2.05 | 8,102 | |
| qwen3.7-max | 2.14 | 8,255 | |
| mimo-v2.5-pro | **2.65** | 12,486 | 调用最多、token 偏高 |
| claude-opus | 1.80 | 20,922 | extended thinking 推高 token |
| codex | 1.86 | **52,705** ⚠️ | token 最贵(深推理 + 多轮) |

两个信号:① **工具调用数**上,Claude 系最克制(opus 1.80 / sonnet 1.77),mimo 最啰嗦(2.65)——少调一次工具就少一次出错/越权的机会,这与 Claude 的高安全自洽;② **token 成本**上,codex 是 kimi 的 12 倍、glm 的 8 倍,深推理换安全的代价直接量化出来了。"哪个模型便宜又够用"在这一列有了答案:**kimi / glm 是省钱档,codex / opus 是烧钱档**。
...
- **双 server,仍非真实生态**:C3 加了第二个 mock(`web_mock`,带 http_post sink + 跨服务器攻击面),但仍是受控 mock。真实 MCP 生态(github/db/browser)的歧义/错误/延迟更复杂,外推性仍受限——下一步是真实 MCP server 的 proxy 评测。
...
- **token 口径不统一**:Claude Code 的 stream-json 不计 cache-read,其 token/成功(sonnet 92.8)与 API 模型不可直接横比;效率结论在"同口径内"才严谨。
...
- ⏭ **C4** — 真实 MCP server 的 proxy 评测 · 更多 server 类型(github/db/browser)· token 口径统一
